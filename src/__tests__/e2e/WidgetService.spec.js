...
describe("Test widget CRUD operations", () => {
  ...
  beforeEach(() => {
    ...
    Database.serialize(() => {
      SqlDb.setupTables();
      Database.run(
        "REPLACE INTO Widgets(WidgetId, Title, DefaultRange, Color_Graph, Frequence) VALUES(1, 'Widget voor gasverbruik', '24h', 'Blue', 40000);"
      );
      Database.run(
        "REPLACE INTO Widgets(WidgetId, Title, DefaultRange, Color_Graph, Frequence) VALUES(2, 'Widget voor kolenverbruik', '48h', 'Red', 40000);"
      );
      Database.run(
        "REPLACE INTO  WidgetSettings (SettingId, Position, ISACTIVE, WidgetId) VALUES(1, 1, 1, 1)"
      );
      Database.run(
        "REPLACE INTO  WidgetSettings (SettingId, Position, ISACTIVE, WidgetId) VALUES(2, 2, 1, 2)"
      );

      Database.run(
        "INSERT OR IGNORE INTO Graphs(GraphId, WidgetId, Name, Measurement) VALUES(1, 1, 'Een grafiek over gasgeneratie', 'm3');"
      );
      Database.run(
        "INSERT OR IGNORE INTO Graphs(GraphId, WidgetId, Name, Measurement) VALUES(2, 1, 'Een grafiek over gasverbruik', 'm3');"
      );

      Database.run(
        "INSERT OR IGNORE INTO Graphs(GraphId, WidgetId, Name, Measurement) VALUES(3, 2, 'Een grafiek over windgeneratie', 'm3');"
      );
    });
  });
