...
const { mapWeatherSqlModelToJsModel } = require("../services/mapping/weather.mapper");
...
const GetOne = async (req, res)=>{
    ...
    try {
        ...
        res.status(404).json({ message: "Search has failed", result: false, error });
    }
...
const Update = async (req, res)=>{
    try {
        ...
        res.status(201).json({ message: "Update is completed", result: newWeather });
    } catch (error) {
        res.status(400).json({ message: "Update has failed", result: false, error });
    }
